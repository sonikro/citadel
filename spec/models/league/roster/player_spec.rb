...
describe League::Roster::Player do
  ...
  it 'validates unique within league' do
    ...
    expect(build(:league_roster_player, roster:, user:)).to be_valid
    expect(build(:league_roster_player, roster: roster2, user:)).to be_valid
    expect(build(:league_roster_player, roster: roster3, user:)).to be_valid

    roster.add_player!(user)

    expect(build(:league_roster_player, roster:, user:)).to be_invalid
    expect(build(:league_roster_player, roster: roster2, user:)).to be_invalid
    expect(build(:league_roster_player, roster: roster3, user:)).to be_invalid
  ...
  it 'creates transfers on creation' do
    ...
    player = create(:league_roster_player, roster:)
